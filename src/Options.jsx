...
class Options extends React.Component {
    ...
    updateInfo() {
        $.get('http://0.0.0.0:3001/api/stores/' + this.state.storeID).done((storeData) => {
          this.setState({storeInfo: storeData[0]});
        });
        $.get('http://0.0.0.0:3001/api/items/' + this.state.itemID).done((itemData) => {
          this.setState({itemInfo: itemData[0]});
        });
        $.get(`http://0.0.0.0:3001/api/inventory/${this.state.storeID}/${this.state.itemID}`).done((stockData) => {
          this.setState({stock: stockData[0].stock});
        });
        $.get(`http://0.0.0.0:3001/api/allStores`).done((stores) => {
    ...
    updateRelatedItems(number = 9) {
      ...
      $.get(`http://0.0.0.0:3001/api/relatedItems/${from}/${to}`).done((items) => {
